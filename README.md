...
6. **Число свободных узлов расчитывается, как**:
   - \(N_free = N - N_CO - N_O - N_Os\) 
...
```mermaid
graph TD
    A[Начало] --> B[Инициализация параметров задачи и переменных]
    B --> C[Установка начальных условий: N_CO, N_O, N_Os]
    C --> D[Инициализация генератора случайных чисел]
    D --> E[Цикл по времени (t < tmax)]
    E --> F{t >= t_out}
    F -->|Да| G[Запись текущих значений в результат]
    G --> H[Обновление t_out]
    F -->|Нет| I[Переход к следующему шагу]
    I --> J[Расчет нормировочного фактора NN1]
    J --> K[Вычисление скоростей процессов (Rate)]
    K --> L[Вычисление суммарной скорости RSum]
    L --> M{RSum == 0?}
    M -->|Да| N[Выход из цикла]
    M -->|Нет| O[Выбор реакции на основе случайного числа]
    O --> P[Выполнение выбранной реакции (адсорбция, десорбция, реакция, переход)]
    P --> Q[Обновление значений переменных N_CO, N_O, N_Os]
    Q --> R[Вычисление времени t]
    R --> S[Переход к следующему шагу в цикле]
    S --> E
    N --> T[Возврат результата]
    T --> U[Конец]
```
